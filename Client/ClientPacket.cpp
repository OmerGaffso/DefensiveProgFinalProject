...
ClientPacketHeader::ClientPacketHeader(uint16_t opCode, uint32_t size, std::array<uint8_t, CLIENT_ID_LENGTH>& id)
    : version(PROTOCOL_VERSION), code(opCode), payloadSize(size), clientId(id) {}

ClientPacket::ClientPacket() : header(), payload() {}

ClientPacket::ClientPacket(uint16_t opCode, const std::vector<uint8_t>& data, std::array<uint8_t, CLIENT_ID_LENGTH>& id)
